...
export const MarkdownLatexRenderer = React.memo(function MarkdownLatexRenderer({
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Début ou Fin de bloc de code ```
    if (line.trim().startsWith('```')) {
      if (inCodeBlock) {
        // Fin de bloc
        elements.push(
          <FencedCodeBlock key={`code-${i}`} code={codeBlockContent.trim()} language={codeBlockLang} />
        );
        inCodeBlock = false;
        codeBlockContent = '';
        codeBlockLang = '';
    ...
    if (line.trim().startsWith('|') && line.trim().endsWith('|')) {
      ...
      // Si c'est la dernière ligne ou que la suivante n'est pas un tableau, on génère la table
      if (i === lines.length - 1 || !lines[i + 1].trim().startsWith('|')) {
        elements.push(<MarkdownTableRenderer key={`table-${i}`} rows={[...tableRows]} />);
        tableRows = [];
      }
      continue;
    }
    ...
    if (line.startsWith('# ')) {
      elements.push(
        <Text key={`h1-${i}`} className="text-base font-extrabold text-zinc-900 dark:text-zinc-50 mt-4 mb-1">
          {line.replace('# ', '')}
        </Text>
      );
      continue;
    ...
    if (line.startsWith('## ')) {
      elements.push(
        <Text key={`h2-${i}`} className="text-sm font-bold text-zinc-900 dark:text-zinc-50 mt-3 mb-1">
          {line.replace('## ', '')}
        </Text>
      );
      continue;
    ...
    if (line.startsWith('### ')) {
      elements.push(
        <Text key={`h3-${i}`} className="text-xs font-bold text-orange-600 dark:text-orange-400 uppercase tracking-wide mt-2.5 mb-1">
          {line.replace('### ', '')}
        </Text>
      );
      continue;
    ...
    if (line.startsWith('- [ ] ')) {
      elements.push(
        <View key={`chk-${i}`} className="flex-row items-center gap-2 my-0.5 ml-1">
          <Text className="text-xs text-zinc-400">☐</Text>
          <InlineTextRenderer text={line.replace('- [ ] ', '')} />
        </View>
      );
      continue;
    }
    if (line.startsWith('- [x] ')) {
      elements.push(
        <View key={`chkx-${i}`} className="flex-row items-center gap-2 my-0.5 ml-1">
          <Text className="text-xs text-orange-500 font-bold">☑</Text>
          <InlineTextRenderer text={line.replace('- [x] ', '')} />
        </View>
      );
      continue;
    }
    ...
    if (line.startsWith('- ') || line.startsWith('* ') || line.startsWith('+ ')) {
      elements.push(
        <View key={`bullet-${i}`} className="flex-row items-start gap-1.5 my-0.5 ml-1.5">
          <Text className="text-xs text-orange-500 font-bold">•</Text>
          <View className="flex-1">
            <InlineTextRenderer text={line.substring(2)} />
          </View>
        </View>
      );
      continue;
    ...
    if (line.startsWith('> ')) {
      elements.push(
        <View key={`quote-${i}`} className="my-1.5 pl-3 border-l-2 border-orange-500 bg-orange-50/50 dark:bg-zinc-800/50 py-1 rounded-r-lg">
          <InlineTextRenderer text={line.replace('> ', '')} baseTextColor="text-zinc-600 dark:text-zinc-300 italic" />
        </View>
      );
      continue;
    }
    ...
    if (line.trim() === '---' || line.trim() === '***') {
      elements.push(<View key={`hr-${i}`} className="h-[0.5px] bg-zinc-200 dark:bg-zinc-800 my-3" />);
      continue;
    }
    ...
    if (line.trim() !== '') {
      elements.push(
        <View key={`p-${i}`} className="my-0.5">
          <InlineTextRenderer text={line} />
        </View>
      );
    }
  }
