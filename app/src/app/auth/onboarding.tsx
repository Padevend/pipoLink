...
export default function OnboardingScreen(): JSX.Element {
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof OnboardingFormValues | 'avatar', string>>>({});
    ...
    <SafeAreaView className="flex-1 bg-background-light dark:bg-background-dark">
      <LinearGradient colors={[...BRAND.gradient]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} className="px-6 py-6">
        ...
        <Text className="mt-3 text-sm text-white/90">
          Complétez votre profil. Les clés de chiffrement sont générées uniquement sur cet appareil principal.
        </Text>
      </LinearGradient>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} className="flex-1">
        <ScrollView className="flex-1 px-5 py-6" keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          <AvatarPicker label="Photo de profil (optionnel)" uri={avatarUri} onChange={setAvatarUri} />
          ...
          <View className="mb-4 mt-4">
            <GenderPicker label="Genre (optionnel)" value={gender} onChange={setGender} error={errors.gender} />
          </View>
          ...
          <Input
            ...
            multiline
            containerClassName="mb-8 min-h-[100px]"
